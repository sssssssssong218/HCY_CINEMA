...
<body>
<%
String[] stillcheck=request.getParameterValues("stillfile");
if(stillcheck!=null){
	int i=0;
	for(String stillfiles:stillcheck){
		if(stillfiles!=null){
		System.out.println(stillfiles);
		}
		i++;
	}
}
...
<%-- <%
		...
		dmDAO.deleteMovieFile(movieCode);
		...
		try {
		    ...
		    String[] selectStill=request.getParameterValues("stillfile");
		    if(selectStill!=null){
		    	int i=0;
		    	for(String stillfiles:selectStill){
		    		if(stillfiles!=null){
		    		File deleteStillFile=new File(saveDir+"/"+movieCode+"_S"+(i+1));
		    		}
		    		i++;
		    	}
		    }
		    String[] posterFile=request.getParameterValues("posterfile");
		    String[] trailerFile=request.getParameterValues("trailerfile");
		    if(posterFile!=null){
		    ...
		    for (String still : hideArr) {
		    	if(still.substring(still.lastIndexOf(".")).equals(".jpg")||still.substring(still.lastIndexOf(".")).equals(".png")){
		    		...
		    		%>
		    		<strong>스틸컷의 확장자는 jpg또는 png만 가능합니다</strong><br/>
		    		<a href="http://localhost/HCY_CINEMA/admin/manageMovie/manage_detail_movie.jsp?movieCode=<%=movieCode%>">뒤로가기</a>
		    ...
		    java.sql.Date sqlDate1 = new java.sql.Date(date1.getTime());
		     dmDAO.updateMovie(movieCode,new AddMovieVO(mname, sqlDate, sqlDate1, plot, Integer.parseInt(runningtime), age, status));
