...
<jsp:include page="../include/headerContents.jsp"/>
	...
	<div id="contents" class="">
        <div id="ctl00_PlaceHolderContent_reserveTitleLine" class="preOrderMovie_wrap">
	 <ul class="preOrderMovie_list even" id="preOrderMovie_list">
		<% 
		...
		List<MovieChartVO> list = SearchDAO.getInstance().selectMovieSearch(search);
		for(MovieChartVO mcVO : list){
			%>
			<li>
						<a href="http://localhost/HCY_CINEMA/common/movie_files/<%=mcVO.getMovieCode() %>_P.jpg %>" class="img_wrap" title="포스터 크게 보기 새창">
						<img src="http://localhost/HCY_CINEMA/common/movie_files/<%=mcVO.getMovieCode() %>_P.jpg" alt="<%=mcVO.getmName()%> 이미지 찾을 수 없음"></a>
						
						<div class="preOrderMovie_info_wrap">
						<strong class="preOrderMovieName"><%=mcVO.getmName()%><i class="cgvIcon etc age<%=mcVO.getMovieRating()=="AL"?"All":mcVO.getMovieRating()%>"><%=mcVO.getMovieRating()=="AL"?"All":mcVO.getMovieRating()%></i></strong>
						<span>예매율 <%=mcVO.getTicketRate() %></span>
						<span><div>
                                            <img src="https://img.cgv.co.kr/R2014/Images/common/egg/eggPreegg.png">
                                            <span><%=mcVO.getStarRating() %>점</span>
                                        </div></span>
						<span><%=mcVO.getReleaseDate() %> 개봉</span>
						<div class="screenType_wrap">
						
						</div>
						<div class="preOrderMovie_explicate"><%=mcVO.getPlot() %></div>
					
					
					<div class="btn_wrap">
						<a href="http://localhost/HCY_CINEMA/user/movieInfo/movie_detail.jsp?movie=<%=mcVO.getMovieCode() %>" class="btn_borderStyle1">상세보기</a>
						<a href="http://localhost/HCY_CINEMA/user/ticketing/ticketing_main.jsp?movie=<%=mcVO.getMovieCode() %>" class="btn_style1">예매하기</a>
					</div>
					</div>
					</li>
			<%
